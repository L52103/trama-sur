...
@Component({
  ...
  template: `
    <section class="hero">
      <img src="/assets/images/hero-trama-sur.png" alt="Prendas de alta calidad Trama Sur en fotografía sobria y cuidada" width="1792" height="1024" fetchpriority="high">
      <div class="hero-copy"><p class="eyebrow">{{content().hero.eyebrow}}</p><h1>{{content().hero.title}}<br><em>{{content().hero.accent}}</em></h1><p>{{content().hero.description}}</p><div><a class="button" routerLink="/coleccion">{{content().hero.ctaLabel}} <app-icon name="arrow"/></a><a class="text-link" routerLink="/legal/manifiesto">Conoce nuestro estándar de calidad</a></div></div>
      <div class="hero-note"><strong>01</strong><span>Confección superior<br>y materiales nobles</span></div>
    </section>
    ...
    <section class="shop-by container"><p class="eyebrow">Encuentra tu estilo</p><h2>Diseño y confección de calidad</h2><div class="category-grid"><a routerLink="/abrigos"><img src="/assets/images/chaqueta-commuter.png" alt="Abrigos y chaquetas de calidad"><span>Abrigos <app-icon name="arrow"/></span></a><a routerLink="/tops"><img src="/assets/images/polera-organica.png" alt="Poleras y tops de algodón orgánico"><span>Tops y Poleras <app-icon name="arrow"/></span></a><a routerLink="/pantalones"><img src="/assets/images/pantalon-travel.png" alt="Pantalones de alta durabilidad"><span>Pantalones <app-icon name="arrow"/></span></a></div></section>
  `,
